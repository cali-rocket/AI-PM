...
5. `Primary Agent -> Tool Connector`: Notion과 Slack(멘션 신호)을 read-only 조회한다.
6. `Primary Agent -> Shared Memory`: 최근 personal task/project summary memory를 조회한다.
...
13. `Primary Agent -> Shared Memory`: 오늘의 우선순위 스냅샷(personal task summary)을 갱신한다.
